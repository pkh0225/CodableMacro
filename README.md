...
### `@Default`도 없고 `var x: T = …`도 없을 때 (타입별 기본값)

`@Default`가 없고 선언부에 **`=` 초기값도 없으면**, 프로퍼티 타입에 따라 아래 **내장 기본값**이 디코딩 폴백 등에 쓰입니다.
...
| 배열 `[Element]` | 빈 배열 (`Array<...>()`) |
| 딕셔너리 `[Key: Value]` | 빈 딕셔너리 (`Dictionary<...>()`) |
| 위에 해당하지 않는 타입 (열거형·커스텀 `struct` 등) | 합성 코드상 폴백은 `nil` — **비옵셔널**이면 컴파일이 맞지 않을 수 있으므로, 그런 타입은 `@Default(...)` 또는 `= 초기값`을 두는 것이 안전합니다. |
